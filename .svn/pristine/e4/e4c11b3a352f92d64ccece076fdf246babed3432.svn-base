...
//        }
    ...
    public Page<Orderwdsjsh> getOrdersjsh(Orderwdsjsh orderwdsjsh,DateTransform dateTransform,String stateCode,Integer serviceUserId,Pageable pageable){ 
    ...
    public Page<Orderwdsjsh> getCommissionerOrdersjshList(Orderwdsjsh orderwdsjsh,DateTransform dateTransform,String stateCode,Pageable pageable){
    ...
    //查询各个产品的申请总数
	public Map  productAmount(){		
		Integer sjshAmount = new Integer(orderwdsjshRepository.getOrderwdsjshAmount());
		Integer wdxydAmount = new Integer(orderwdxydRepository.getOrderwdxydAmount());
		Integer rzzlAmount = new Integer(orderrzzlRepository.getOrderrzzlAmount());
		Integer mdbtAmount = new Integer(ordermdbtRepository.getOrdermdbtAmount());
		Map<String , Integer>   map = new  HashMap<>();
		map.put("网点随借随还", sjshAmount);
		map.put("网点信用贷", wdxydAmount);
		map.put("融资租赁", rzzlAmount);
		map.put("面单白条", mdbtAmount);
		return map;
	}
    
	/*//修改身份证
	public void dochangeID(Orderwdsjsh orderwdsjsh){			
		Orderwdsjsh sjsh = orderwdsjshRepository.findByApplyIdentityNoAndState(orderwdsjsh.getApplyIdentityNo());
		if(sjsh != null){
			throw new FailReturnObject(1330, "该身份证已经被使用！", ReturnObject.Level.WARNING);
		}
	}*/
