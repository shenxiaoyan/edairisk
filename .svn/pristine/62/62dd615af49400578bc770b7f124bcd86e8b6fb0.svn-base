...
public class TIMGroupService extends AbstractAuditorService<TIMGroup,TIMGroupState,TIMGroupAct,TIMGroupLog> {
	...
	public void sqlInit() {
			...
			long findAll = groupActRepository.count();
	        if(findAll == 0 ){
			...
			TIMGroupAct save6 = groupActRepository.save(new TIMGroupAct("部门的列表","listOwnDepartment",60,ActGroup.READ));
			TIMGroupAct save7 = groupActRepository.save(new TIMGroupAct("部门和下属部门的列表","listOwnDepartmentAndChildren",70,ActGroup.READ));
			TIMGroupAct save8 = groupActRepository.save(new TIMGroupAct("通知其他人","noticeOther",80,ActGroup.OPERATE));
			TIMGroupAct save9 = groupActRepository.save(new TIMGroupAct("通知操作者","noticeActUser",90,ActGroup.OPERATE));
			TIMGroupAct save10 = groupActRepository.save(new TIMGroupAct("通知展示人","noticeShowUser",100,ActGroup.OPERATE));
			
	
			groupStateRepository.save(new TIMGroupState("已创建",0,"CREATED"));
			TIMGroupState groupState = new TIMGroupState("有效",100,"ENABLED");
			groupState.setActs(Arrays.asList(save1,save2,save3,save4,save5,save6,save7).stream().collect(Collectors.toSet()));
