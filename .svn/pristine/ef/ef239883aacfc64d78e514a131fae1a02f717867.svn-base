...
public class MenuService extends AbstractAuditorService<Menu,MenuState,MenuAct,MenuLog> {
	...
	public void sqlInit() {
			...
			long findAll = menuActRepository.count();
	        if(findAll == 0 ){
			...
			MenuAct save7 = menuActRepository.save(new MenuAct("部门和下属部门的列表","listOwnDepartmentAndChildren",70,ActGroup.READ));
			...
			menuState.setActs(Arrays.asList(save1,save2,save3,save4,save5,save6,save7).stream().collect(Collectors.toSet()));
