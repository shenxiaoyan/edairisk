...
public interface OrderwdxydRepository extends WorkflowEntityRepository<Orderwdxyd>,SpecificationQueryRepository<Orderwdxyd>{
...
//		public Page<Orderwdxyd> getOrderwdxydByStateCodeAndUser(@Param("stateCode")String stateCode,@Param("userid")String userid, Pageable pageable);
    ...
    @Query("select o from Orderwdxyd o  where o.createdBy.id=:userid ")
    public List<Orderwdxyd> findByCreateBy( @Param("userid") Integer userid);
    
    @Query("select o from Orderwdxyd o where o.person.id=:personId ")
    public Orderwdxyd findByPersonId(@Param("personId") Integer personId);
    
    @Query("select count(*) from Orderwdxyd ")
    public int getOrderwdxydAmount();
