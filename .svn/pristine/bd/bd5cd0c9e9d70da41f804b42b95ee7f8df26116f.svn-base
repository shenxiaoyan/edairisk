...
//		sCfg.setPrivateKey("");
		System.out.println("--e签初始化...");
		EsignsdkService sdk = EsignsdkServiceFactory.instance();
		Result result = sdk.init(proCfg, httpConCfg, sCfg);
		if (0 != result.getErrCode()) {
			throw new FailReturnObject(1684,"--e签初始化失败：errCode=" + result.getErrCode() + " msg=" + result.getMsg(), ReturnObject.Level.FATAL);
		} else {
			System.out.println("--e签初始化成功！errCode=" + result.getErrCode() + " msg=" + result.getMsg());
		}
	}
...
//		SignPDFStreamBean signPDFStreamBean = setSignPDFStreamBean(FileHelper.getBytes(srcPdfFile));
		SignPDFStreamBean signPDFStreamBean = setSignPDFStreamBean(FileHelper.getByteByUrl(srcPdfFile));//--Jax
		...
		if (0 != fileDigestSignResult.getErrCode()) {
			throw new FailReturnObject(1563,"平台自身PDF摘要签署（文件流）失败，errCode=" + fileDigestSignResult.getErrCode() + " msg="+fileDigestSignResult.getMsg(), ReturnObject.Level.FATAL);
		} else {
...
//		organizeBean.setMobile("188****0787");
		...
		organizeBean.setOrganCode("51600595-3");
		// 公司地址,可空
