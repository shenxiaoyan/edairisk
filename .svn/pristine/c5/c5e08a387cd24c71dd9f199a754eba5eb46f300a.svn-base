...
//	}
	...
	public void setNoticeRoles(String noticeRoles) {
		if(noticeRoles!=null){
			if(!noticeRoles.matches("^[\\d]+(-[\\d]+)*$")){
				throw new FailReturnObject(1534,"通知的角色格式错误，请从新配置", ReturnObject.Level.INFO);
			}
		}
	...
	public static enum ActGroup{
		@Info(label="列表普通操作",placeholder="",tip="",help="列表中普通操作",secret="")
		LIST_OPERATE,
		...
		FILE_PACKAGE,
		@Info(label="其他操作",placeholder="",tip="",help="一些配套操作",secret="")	
		OTHER,
		@Info(label="非按钮类行为",placeholder="",tip="",help="非按钮类行为",secret="")
		NOT_BUTTON_UPDATE,
		@Info(label="合同类行为",placeholder="",tip="",help="上传合同等",secret="")
		FILE_CONTRACT
