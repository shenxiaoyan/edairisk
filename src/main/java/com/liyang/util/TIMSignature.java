	...
	public static GenTLSSignatureResult generate(String username) {
		...
		try {
			...
			if (0 == result.urlSig.length()) {
				throw new FailReturnObject(900, "GenTLSSignatureEx failed: " + result.errMessage,com.liyang.util.ReturnObject.Level.FATAL);
			}
			return result;
		} catch (IOException e) {
	...
	public static CheckTLSSignatureResult check(String sig, String username) throws JSONException {
		...
		try {
			...
			if (checkResult.verifyResult == false) {
				throw new FailReturnObject(910, "CheckTLSSignature failed: " + checkResult.errMessage,com.liyang.util.ReturnObject.Level.FATAL);
			}
			return checkResult;
