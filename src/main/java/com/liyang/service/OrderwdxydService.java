...
public class OrderwdxydService extends AbstractWorkflowService<Orderwdxyd, OrderwdxydWorkflow, OrderwdxydAct, OrderwdxydState, OrderwdxydLog, OrderwdxydFile>{
	...
	public void sqlInit() {
			...
			long findAll = orderwdxydActRepository.count();
	        if(findAll == 0 ){
	...
	public void doCreate(Orderwdxyd orderwdxyd) {
		...
		orderwdxyd.setOrderNo(ProductUtil.modify(orderwdxyd.getProduct().getLabel(), OrderTypeEnum.ORDER));
		...
		smsService.sendCommitMessage(orderwdxyd.getOrderNo(), "产品申请", "网点信用贷");
	}
	@Override
...
//		orderwdxyd.setDistribution(true);
		orderwdxyd.setIsDistribution(true);;
	...
	public void doFailed(Orderwdxyd orderwdxyd) {
		...
		String content = xinGePushService.getYiDaiFailedContent(orderwdxyd.getProduct().getLabel());
        xinGePushService.pushAppMessage(orderwdxyd.getCreatedBy(), orderwdxyd.getProduct(), "审核进度通知", content,1);
	}
    ...
    //根据条件动态查询Orderwdxyd并分页
    public Page<Orderwdxyd> getOrderwdxyd(Orderwdxyd orderwdxyd,DateTransform dateTransform,String stateCode,Integer serviceUserId,Pageable pageable){ 
	    ...
	    queryExpression.add(QueryExpSpecificationBuilder.eq("realName", orderwdxyd.getRealName(), true));
		queryExpression.add(QueryExpSpecificationBuilder.eq("applyMobile", orderwdxyd.getApplyMobile(), true));
		queryExpression.add(QueryExpSpecificationBuilder.eq("orderNo", orderwdxyd.getOrderNo(), true));
		...
		return orderwdxydRepository.findAll(queryExpression,pageable);	
    }
    
    //根据条件动态查询Orderwdxyd并分页(风控专员专用)
    public Page<Orderwdxyd> getCommissionerListOrderwdxyd(Orderwdxyd orderwdxyd,DateTransform dateTransform,String stateCode,Pageable pageable){ 
    	
    	GenericQueryExpSpecification<Orderwdxyd> queryExpression = new GenericQueryExpSpecification<Orderwdxyd>();
    	queryExpression.add(QueryExpSpecificationBuilder.eq("state.stateCode", stateCode, true));
    	queryExpression.add(QueryExpSpecificationBuilder.eq("realName", orderwdxyd.getRealName(), true));
    	queryExpression.add(QueryExpSpecificationBuilder.eq("applyMobile", orderwdxyd.getApplyMobile(), true));
    	queryExpression.add(QueryExpSpecificationBuilder.eq("serviceUser.id", orderwdxyd.getServiceUser().getId(), true));
    	queryExpression.add(QueryExpSpecificationBuilder.eq("orderNo", orderwdxyd.getOrderNo(), true));
    	queryExpression.add(QueryExpSpecificationBuilder.gt("createdAt", dateTransform.getCreateDateTime(), true));
    	queryExpression.add(QueryExpSpecificationBuilder.lt("createdAt", dateTransform.getEndDateTime(), true));
